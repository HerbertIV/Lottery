                                    ...
                                    <div id="docs-card-content" class="flex items-start gap-6 flex-col">
                                        ...
                                        @isset($members)
                                            @foreach($members as $member)
                                                <div class="flex">
                                                    <form method="POST"
                                                          action="{{ route('lottery-session.member.destroy', ['lotterySession' => $lotterySessionName, 'member' => $member]) }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="submit"
                                                    ...
                                                    @if ($member->canDraw())
                                                        <form method="GET"
                                                              action="{{ route('lottery-session.member.send-drawn-member', ['lotterySession' => $lotterySessionName, 'member' => $member]) }}">
                                                            @csrf
                                                            @method('get')
...
{{--                                                            </button>--}}
                    ...
                    @auth
                        ...
                        >
                            ...
                            <form class="flex w-full flex-wrap lg:flex-nowrap" method="POST" action="{{ route('lottery-session.member.store', ['lotterySession' => $lotterySessionName]) }}">
                    ...
                    >
                        @auth
                            ...
                            <form class="flex w-full flex-wrap lg:flex-nowrap" method="POST" action="{{ route('lottery-session.turn.store', ['lotterySession' => $lotterySessionName]) }}">
                                <div class="pt-3 sm:pt-5 w-full">
